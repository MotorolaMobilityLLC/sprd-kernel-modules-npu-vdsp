...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "[VDSP]vdsp_library_loader: %d %d %s : "\
...
static int32_t libinfo_list_remove(struct libinfo_list *list,int32_t pos)
{
	...
	if(pos == 0){
		...
		list->number--;
		if(node->element) {
			pr_info("pos is 0 free element:%p\n", node->element);
			vfree(node->element);
		}
		pr_info("pos is 0 free new node:%p\n", node);
		vfree(node);
		return LIST_SUCCESS;
	}
	...
	list->number--;
	if(del_node->element){
		pr_info("free element:%p\n", del_node->element);
		vfree(del_node->element);
	}
	pr_info("free new node:%p\n", del_node);
	vfree(del_node);
	return LIST_SUCCESS;
}
...
static int32_t xrp_library_load_internal(struct xvp *xvp , const char* buffer , const char *libname)
{
	...
	if((NULL == lib_ion_mem) || (NULL == libinfo_ion_mem)) {
		pr_err("[ERROR]vmalloc lib_ion_mem failed libionmem:%p , libinfoionmem:%p\n", lib_ion_mem , libinfo_ion_mem);
		if(NULL != lib_ion_mem)
			vfree(lib_ion_mem);
	...
	pr_info("load pi library buffer:%p , kpaddr:%lx, kvaddr:%p , kpaddr_libinfo:%lx , kvaddr_libinfo:%p\n",
		buffer , (unsigned long) kpaddr , kvaddr , (unsigned long) kpaddr_libinfo , kvaddr_libinfo);
	...
	if(result == 0)
	{
		...
		vfree(libinfo_ion_mem);
		/**/
		pr_err("[ERROR]xtlib_host_load_pi_library failed %d\n", result);
		return -1;
	}
...
int32_t xrp_library_decrelease(struct xvp *xvp , const char *libname)
{
	...
	if(i < libinfo_list_size(&(xvp->load_lib.lib_list))) {
		if(libinfo->load_count != 0)
			libinfo->load_count--;
		pr_info("load_count:%d, libname:%s\n", libinfo->load_count, libname);
		if(libinfo->load_count == 0) {
			...
			pr_err("release failed loadcount:%d , libname:%s\n" , libinfo->load_count , libname);
		}
		ret = 0; /*loaded*/
	} else {
		pr_err("[ERROR]not find lib [%s], may be some error\n", libname);
		ret = 1;
	}
	return ret;
}
...
static int32_t xrp_library_kmap_ionbuf(struct ion_buf *ionbuf , __u8 **buffer , struct dma_buf **dmabuf)
{
	...
	*buffer = NULL;
	if((ionbuf == NULL) || (ionbuf->mfd[0] < 0)) {
		pr_err("[ERROR]ion dsp pool is NULL return error\n");
		return -EINVAL;
	}
	...
	*dmabuf = dma_buf;
	...
	pr_info("[kmap]mfd:%d , dev:%p , map kernel vaddr is:%p , mapcount:%d\n", ionbuf->mfd[0] , ionbuf->dev , *buffer  , mapcount);
	if(NULL == *buffer)
		return -EFAULT;
	return 0;
}
	...
	if(0 == strcmp(rq->nsid , LIBRARY_LOAD_UNLOAD_NSID) && (indata_size >= LIBRARY_CMD_LOAD_UNLOAD_INPUTSIZE)) {
		...
		if(libinfo != NULL) {
			...
			*((unsigned int*)((__u8 *)inputbuffer + 40)) = libinfo->pil_info;
			pr_info("p:%p , p+40 value:%x , pil_info:%x\n", inputbuffer , *((unsigned int*)((__u8 *)inputbuffer + 40)) ,libinfo->pil_info);
			xrp_library_kunmap_ionbuf(dmabuf);
			wmb();
		} else {
			pr_err("[ERROR]xrp_library_getlibinfo failed\n");
			ret = -EINVAL;
		}
	} else {
		pr_err("[ERROR]nisid is not unload\n");
		ret = -EINVAL;
	}
	pr_info("[UNLOAD][OUT]lib unload end, ret[%d]\n", ret);
	return ret;
}
	...
	if(0 == strcmp(rq->nsid , LIBRARY_LOAD_UNLOAD_NSID)) {
		...
		if(XRP_LOAD_LIB_FLAG == load_flag) {
			...
			if(i < libinfo_list_size(&(xvp->load_lib.lib_list))) {
				...
				if(ret != 0) {
					pr_err("[ERROR]xrp_library_parase_libbuffer failed\n");
					vfree(tempbuffer);
					return -EINVAL;
				}
				...
				if(NULL == libinfo) {
					...
					*((uint32_t*)(input_ptr+LIBRARY_CMD_PIL_INFO_OFFSET)) = libinfo->pil_info;
					pr_info("load input param nsid:%s , loadflag:%d , libname:%s , pil_info:%x , indata_size:%d\n",
						rq->nsid, load_flag , libname , libinfo->pil_info , indata_size);
					sprintf(outlibname , "%s" , libname);
				}
				ret = xrp_library_kmap_ionbuf(&rq->ion_in_buf,&inputbuffer ,&dmabuf);
				if(ret != 0) {
					pr_err("[ERROR]xrp_library_kmap_ionbuf input buffer failed\n");
					vfree(tempbuffer);
					xrp_library_decrelease(xvp , libname);
					return -EFAULT;
				}
		...
		vfree(tempbuffer);
		pr_info("[LOAD]lib load end, ret[%d]\n", ret);
		return ret;
	} else
		return 0;
}
...
static int32_t xrp_register_libs(struct xvp *xvp , struct xrp_comm *comm)
{
	...
	if(ret != 0) {
		pr_info("alloc input_ion_buf failed\n");
		return -ENOMEM;
	}
...
int32_t xrp_pre_process_request(struct xvp *xvp , struct xrp_request *rq , enum load_unload_flag loadflag, char *libname)
{
	...
	if(loadflag == XRP_LOAD_LIB_FLAG) {
		...
		if(lib_result == 0) {
			...
			if(load_count > 0) {
				pr_info("needn't unload because load count is not zero\n");
				return -ENXIO;
			} else if(0 == load_count){
#ifndef USE_RE_REGISTER_LIB
				if(libinfo != NULL) {
					if(libinfo->lib_state == XRP_LIBRARY_MISSED_STATE) {
						pr_info("xrp_library_unload libname:%s is missed state, release here\n" , libname);
						xrp_library_decrelease(xvp , libname);
						return -ENXIO;
					}
				}
				...
				pr_err("[ERROR]error decrease count error:%d , libname:%s\n", load_count , libname);
				return -ENXIO;
			}
			if(libinfo != NULL)
			...
			pr_err("[ERROR]XRP_UNLOAD_LIB_FLAG , get libname error , libname:%s , loadflag:%d\n",
				libname , loadflag);
			return -EINVAL;
		}
	}
		...
		if(libinfo != NULL) {
			...
			pr_info("not loaded libinfo\n");
		}
		pr_info("Command libname:%s\n", rq->nsid);
		return 0;
	}
}
int post_process_request(struct xvp *xvp , struct xrp_request *rq , const char* libname , enum load_unload_flag load_flag , int32_t resultflag)
{
	...
	if(load_flag == XRP_LOAD_LIB_FLAG) {
		...
		pr_info("lib:%s , process cmd over\n", rq->nsid);
	}
	pr_info("[OUT]libinfo:%p\n", libinfo);
	return ret;
}
